<template>
    ...
    <div class="gulu-tabs-nav" ref="container">
        ...
        :ref="el => {if ( t == selected2) selectedItem = el}"
        :class="{selected2: t == selected2}" @click="select(t)">{{t}}</div>
  ...
  //通过vue的插槽的特性构建的tabs
    ...
    setup(props, context) {
      ...
      const select = (title: String) =>{
        context.emit('update:selected2', title)
      }
      const current = computed(() => {
        return defaults.find(tag => tag.props.title == props.selected2)
      })
      return {
